...
class BiEncoderNllLoss(object):
    def calc(
        ...
        ####################
        new_positive = torch.tensor(sorted(positive_idx_per_question * 2))
        ###################
        
        loss = F.nll_loss(
            softmax_scores,
            new_positive.to(softmax_scores.device),
            reduction="mean",
        )
        ...
        correct_predictions_count = (
            max_idxs == new_positive.to(max_idxs.device)
        ).sum()

        correct_predictions_count = correct_predictions_count / 2
