...
def create_biencoder_input(
    ...
    for sample in samples:
        ...
        ###############
        for n in range(number_of_qus) :
            paraphrase = de2en.translate(en2de.translate(question, \
                sampling=True, temperature=temperature), sampling=True, temperature=temperature)
            
            question_tensors.append(tensorizer.text_to_tensor(paraphrase) )
        ###############
...
class BiEncoderTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.en2de = torch.hub.load('pytorch/fairseq', 'transformer.wmt19.en-de.single_model', tokenizer='moses', bpe='fastbpe').cuda()
        self.de2en = torch.hub.load('pytorch/fairseq', 'transformer.wmt19.de-en.single_model', tokenizer='moses', bpe='fastbpe').cuda()

        ######################
        self.number_of_qus = self.args.number_of_qus
        self.temperature = self.args.temperature
        ######################

        filehandler = logging.FileHandler(f'{args.temperature}.txt')
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(
            ...
            biencoder_batch = create_biencoder_input(
                samples_batch,
                self.tensorizer,
                True,
                num_hard_negatives,
                num_other_negatives,
                True, # shuffle=True,
                args.shuffle_positive_ctx,  # shuffle_positives=args.shuffle_positive_ctx,
                self.en2de,
                self.de2en,
                self.number_of_qus,
                self.temperature,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='enter the number of question',
    )
